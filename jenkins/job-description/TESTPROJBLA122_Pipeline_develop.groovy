...
def job_config = [
        job: [
                name: "TESTPROJBLA122_Pipeline_develop",
                agent: "maven"
        ],
        git: [
                branch: "develop"
    ...
    stages {
        ...
        stage('Checkout') {
            steps {
                echo "checkout"
                script {
                   jslGit.checkout( config, "kkanto", "TESTPROJBLA122", job_config.git.branch)
               }
            }    
        }
        stage('Build') {
...
def job = pipelineJob("TESTPROJBLA122_Pipeline_develop");
