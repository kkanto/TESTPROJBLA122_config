...
def job_config = [
    job: [
        name: "TESTPROJBLA122_Feature_Build",
        agent: "maven"
    ]
]
def config = [
...
def customLib = library identifier: 'TESTPROJBLA122_JSL@develop', retriever: modernSCM(
  [\$class: 'GitSCMSource',
   remote: 'https://github.developer.allianz.io/kkanto/TESTPROJBLA122_lib.git',
   credentialsId: 'git-token-credentials']) 
...
def jslCustom     = customLib.de.allianz.TESTPROJBLA122.new()
    ...
    stage('Prepare') {
      steps {
        ...
        script {
          ...
          jslGit.checkout( config, "kkanto", "TESTPROJBLA122", branch_name)
          }
        }
      }
stage('Build') {
...
"""
def job = pipelineJob("TESTPROJBLA122_Feature_Build");
...
job.with {
    ...
    authenticationToken('5dDsF7AF0ciMKK3cWSa0dELFomaTkH7h')
